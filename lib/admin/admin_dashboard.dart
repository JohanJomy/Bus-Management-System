...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  late final List<Widget> _pages = [
    ...
    const Center(child: Text("Reports Page", style: TextStyle(fontSize: 24, color: Colors.black))), 
    const Center(child: Text("Settings Page", style: TextStyle(fontSize: 24, color: Colors.black))), 
  ];
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 16),
          const CircleAvatar(radius: 16, backgroundColor: Colors.blueAccent, child: Icon(Icons.person, size: 20, color: Colors.white)),
        ],
      ),
    );
  }
...
class MetricsGrid extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GridView.count(
      ...
      childAspectRatio: 2,
      children: const [
        MetricCard(title: "ACTIVE BUSES", value: "42", icon: Icons.directions_bus),
        MetricCard(title: "TOTAL STUDENTS", value: "1,250", icon: Icons.group),
        MetricCard(title: "PENDING FEES", value: "\$12k", icon: Icons.warning, color: Colors.red),
      ...
      child: Column(
        children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            Text("${(progress * 100).toInt()}%", style: const TextStyle(color: Colors.black, fontWeight: FontWeight.bold)),
          ]),
          const SizedBox(height: 8),
          LinearProgressIndicator(value: progress, color: const Color(0xFF195DE6), backgroundColor: Color(0xFFF1F5F9)),
        ],
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return ClipRRect(
      ...
      child: SizedBox(
        height: 300,
        child: FlutterMap(
          options: const MapOptions(initialCenter: LatLng(9.5107, 76.5511), initialZoom: 15),
          children: [
            TileLayer(urlTemplate: 'https://tile.openstreetmap.org/{z}/{x}/{y}.png'),
            const MarkerLayer(markers: [
               Marker(point: LatLng(9.5107, 76.5511), child: Icon(Icons.directions_bus, color: Color(0xFF195DE6))),
            ]),
          ],
        ),
      ),
  ...
  Widget _logoHeader() {
    return Padding(
      ...
      child: Row(children: [
        ...
        const Text("BusAdmin Pro", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18, color: Colors.black)),
      ]),
    );
  }
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: ListTile(
          ...
          title: Text(label, style: TextStyle(color: isActive ? Colors.white : Colors.black87, fontWeight: FontWeight.w600)),
        ),
      ),
    );
  }
